...
class LearnableDualDropout(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, *rest, D = x.shape
        if self.training:
            # Gumbel-Softmax 采样近似 0/1
            gate1 = F.gumbel_softmax(
                torch.stack([self.logit1, -self.logit1], dim=-1), tau=self.temp, hard=True
            )[..., 0]  # [D] -> 0/1
            gate2 = F.gumbel_softmax(
                torch.stack([self.logit2, -self.logit2], tau=self.temp, hard=True
            )[..., 0])
            # 使 gate1 + gate2 期望为 1
            gate2 = 1.0 - gate1
        else:

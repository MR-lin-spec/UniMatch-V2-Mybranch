...
def jensen_shannon_divergence(log_p, log_q):
    ...
    Input: log_p, log_q of shape (N, C, H, W)
    Output: scalar JSD loss (mean over spatial and batch)
    """
    p = torch.exp(log_p)
    q = torch.exp(log_q)
    m = 0.5 * (p + q)
...
def main():
    ...
    trainloader_u = DataLoader(trainset_u, batch_size=cfg['batch_size'], pin_memory=True, num_workers=10, drop_last=True, sampler=trainsampler_u)
